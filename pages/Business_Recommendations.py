...
def generate_initial_recommendation(business_data: pd.DataFrame) -> str:
    try:
        message = ""
        ...
        Logger.error(f"Error generating initial recommendation: {str(e)}")
        raise


def generate_follow_up_response(chat_history: List[Dict[str, Any]]) -> str:
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
